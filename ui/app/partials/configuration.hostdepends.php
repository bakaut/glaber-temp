...
$tab = (new CFormGrid())
    ...
    ->addItem([
        ...
        new CFormField(
            getDependencyTable($data['depends'], $readonly)
        ),
    ]);

$tab->show();

function getDependencyTable(array $depends, $readonly): CTable
{
    $table = (new CTable())
        ->setId('depends_list')
    ...
    foreach ($depends as $dep) {
        $table->addRow(getDependencyRow($dep, $readonly),'form_row');
    }
...
function getDependencyRow(array $dep, $readonly): CRow
{
