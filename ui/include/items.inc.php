...
function item_type2str($type = null) {
	$types = [
		...
		ITEM_TYPE_TRAP_REGEXP => _('Regexp trapper'),
...
function orderItemsByDelay(array &$items, $sortorder, array $options){
	...
	foreach ($items as &$item) {
		if (in_array($item['type'], [ITEM_TYPE_TRAPPER, ITEM_TYPE_TRAP_REGEXP, ITEM_TYPE_DEPENDENT])
...
function get_preprocessing_types($type = null, $grouped = true, array $supported_types = []) {
	$types = [
		...
		GLB_PREPROC_LOCAL_DISPATCH_REGEXP => [
			'group' => _('Dispatch'),
			'name' => _('Dispatch within host by JSON by regexp')
		],
...
function normalizeItemPreprocessingSteps(array $preprocessing): array {
	foreach ($preprocessing as &$step) {
		switch ($step['type']) {
			...
			case GLB_PREPROC_LOCAL_DISPATCH_REGEXP:
			case GLB_PREPROC_DISCOVERY_PREPARE:
			case GLB_PREPROC_THROTTLE_TIMED_VALUE_AGG:
			case GLB_PREPROC_DISPATCH_LOCAL:
				$step['params'] = implode("\n", $step['params']);
				break;
