...
$readonly = $data['limited'] || false == $can_edit;
...
foreach ($data['delay_flex'] as $i => $delay_flex) {
	...
	if ($delay_flex['type'] == ITEM_DELAY_FLEXIBLE) {
		$delay_input = (new CTextBox('delay_flex['.$i.'][delay]', $delay_flex['delay']))
			->setAttribute('placeholder', ZBX_ITEM_FLEXIBLE_DELAY_DEFAULT)->setReadonly($readonly);
		$period_input = (new CTextBox('delay_flex['.$i.'][period]', $delay_flex['period']))
			->setAttribute('placeholder', ZBX_DEFAULT_INTERVAL)->setReadonly($readonly);
		...
		$schedule_input = (new CTextBox('delay_flex['.$i.'][schedule]', $delay_flex['schedule']))
			->setAttribute('placeholder', ZBX_ITEM_SCHEDULING_DEFAULT)->setReadonly($readonly);
	}
...
$item_tab
	->addItem([
		...
		(new CFormField((new CCheckBox('allow_traps', HTTPCHECK_ALLOW_TRAPS_ON))
			->setChecked($data['allow_traps'] == HTTPCHECK_ALLOW_TRAPS_ON)->setReadonly($readonly)
	...
	->addItem([
		...
		(new CFormField((new CTextBox('trapper_hosts', $data['trapper_hosts']))
			->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)->setReadonly($readonly)
	...
	->addItem([
		...
		new CFormField((new CCheckBox('status', ITEM_STATUS_ACTIVE))->setChecked($data['status'] == ITEM_STATUS_ACTIVE)->setReadonly($readonly))  
	])
	->addItem([
...
if ($data['itemid'] != 0) {
	$item_tabs->setFooter(makeFormFooter(
		(new CSubmit('update', _('Update')))->setEnabled(!$readonly), [
			(new CSubmit('clone', _('Clone')))->setEnabled(!$readonly),
