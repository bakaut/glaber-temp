...
$triggersFormList
	...
	->addRow(
		...
		(new CTextBox('opdata', $data['opdata'], $data['limited']))->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)->setReadonly(!$can_edit)
	)
	->addRow(
		_('Severity'),
		(new CSeverity('priority', (int) $data['priority']))->setReadonly(!$can_edit)
	);
...
$add_expression_button = (new CButton('insert', $data['expression_constructor'] == IM_TREE ? _('Edit') : _('Add')))
	...
	')
	->removeId();
if ($data['limited'] || false == $can_edit) {
...
$expression_row = [
	(new CTextArea(
		...
		['readonly' => $data['expression_field_readonly'] || false == $can_edit]
	))
...
$triggersFormList
	...
	->addRow(_('Create enabled'), (new CCheckBox('status'))->setChecked($status)->setEnabled($can_edit))
	->addRow(_('Discover'),
	...
	if ($dependency['flags'] == ZBX_FLAG_DISCOVERY_PROTOTYPE) {
		$description = (new CLink($depTriggerDescription,
			(new CUrl('trigger_prototypes.php'))
				...
				->setArgument('context', $data['context'])
		))->setTarget('_blank')->setEnabled($can_edit);
	}
...
if (!empty($data['triggerid'])) {
	...
	$triggersTab->setFooter(makeFormFooter(
		(new CSubmit('update', _('Update')))->setEnabled($can_edit), [
			(new CSubmit('clone', _('Clone')))->setEnabled($can_edit),
			$deleteButton,
			$cancelButton
		]
	));
