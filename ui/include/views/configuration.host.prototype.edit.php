...
$host_tab->addRow(
	...
	(new CMultiSelect([
		...
		'disabled' => (bool) $host_prototype['templateid'] || false == $can_edit,
		'data' => $data['groups_ms'],
		'popup' => [
			'parameters' => [
				'srctbl' => 'host_groups',
		...
		(new CTable())
			...
			->addRow(
				(new CRow())
					...
					->addItem(
						(new CCol(
							(new CButton('group_prototype_add', _('Add')))->addClass(ZBX_STYLE_BTN_LINK)->setEnabled($can_edit)
...
$host_tab->addRow(
	...
	[
		(new CRadioButtonList('custom_interfaces', (int) $host_prototype['custom_interfaces']))
			...
			->setModern(true)
			->setReadonly($host_prototype['templateid'] != 0 || false == $can_edit),
		...
		new CDiv(
			(new CButton('interface-add', _('Add')))
				...
				->setEnabled($host_prototype['templateid'] == 0 && $can_edit)
		)
	]
);
...
$tabs->addTab('tags-tab', _('Tags'),
	new CPartial('configuration.tags.tab', [
		...
		'tags' => $data['tags'],
		'readonly' => $data['readonly'] || false == $can_edit,
		'tabs_id' => 'tabs',
		...
		->addRow(
			...
			new CPartial(
				...
				[
					...
					'readonly' => $data['templates'] || false == $can_edit
				]
			),
			'macros_container'
		),
