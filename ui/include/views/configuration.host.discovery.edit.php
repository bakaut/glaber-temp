...
$item_tab
	...
	->addItem([
		...
		new CFormField((new CSelect('type'))
			...
			->setReadonly($data['limited'] || false == $can_edit)
...
$item_tab
	...
	->addItem([
		...
		new CFormField((new CTextArea('description', $data['description']))->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)->setReadonly(!$can_edit))
	])
	->addItem([
		new CLabel(_('Enabled'), 'status'),
		new CFormField((new CCheckBox('status', ITEM_STATUS_ACTIVE))->setChecked($data['status'] == ITEM_STATUS_ACTIVE)->setEnabled($can_edit))
	]);
	...
	->addTab('preprocTab', _('Preprocessing'),
		(new CFormGrid())
			...
			->addItem([
				...
				new CFormField(
					getItemPreprocessing($data['preprocessing'], $data['limited'] || false == $can_edit, $data['preprocessing_types'])
				)
			]),
...
if (!empty($data['itemid'])) {
	...
	if ($data['host']['status'] != HOST_STATUS_TEMPLATE) {
		$buttons[] = (new CSimpleButton(_('Execute now')))
			->setEnabled(
					in_array($data['item']['type'], checkNowAllowedTypes())
	...
	$buttons[] = (new CButtonDelete(_('Delete discovery rule?'), url_params(['form', 'itemid', 'hostid', 'context']).
		...
		'context'
	))->setEnabled(!$data['limited'] && $can_edit);
	$buttons[] = new CButtonCancel(url_param('context'));

	$form_actions = new CFormActions($can_edit? (new CSubmit('update', _('Update'))) : null , $buttons);
}
else {
