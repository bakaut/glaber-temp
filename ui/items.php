...
$fields = [
	...
	'delay' =>						[T_ZBX_TU, O_OPT, P_ALLOW_USER_MACRO, null,
										'(isset({add}) || isset({update})) && isset({type})'.
											' && ({type} != '.ITEM_TYPE_TRAPPER.' && {type} != '.ITEM_TYPE_TRAP_REGEXP.
											' && {type} != '.ITEM_TYPE_DEPENDENT.
	...
	'type' =>						[T_ZBX_INT, O_OPT, null,
										IN([-1, ITEM_TYPE_ZABBIX, ITEM_TYPE_TRAPPER, ITEM_TYPE_SIMPLE,
											...
											ITEM_TYPE_TELNET, ITEM_TYPE_JMX, ITEM_TYPE_CALCULATED, ITEM_TYPE_TRAP_REGEXP,
...
$can_edit = CWebUser::checkAccess(CRoleHelper::UI_CONFIGURATION_HOSTS);
...
// list of items
	...
	if (hasRequest('filter_delay')) {
		...
		if ($filter_delay !== '') {
			...
			elseif ($filter_type == ITEM_TYPE_TRAPPER || $filter_type == ITEM_TYPE_TRAP_REGEXP
	...
	if ($data['items']) {
		...
		foreach ($data['items'] as &$item) {
			...
			if ($item['type'] == ITEM_TYPE_TRAPPER || $item['type'] == ITEM_TYPE_TRAP_REGEXP
