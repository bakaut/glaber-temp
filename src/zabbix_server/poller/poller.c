		...
		const zbx_config_comms_args_t *config_comms, int config_startup_time)
{
	...
	switch (item->type)
	{
		...
		case ITEM_TYPE_SCRIPT:
			DEBUG_ITEM(item->itemid, "Running the script '%s' with params '%s'",
			 	item->params, item->script_params);
				
			if (NULL != item->params && NULL != strstr(item->params,"#!")) {
				DEBUG_ITEM(item->itemid, "Executing as shell script");
				res = get_value_shell_script(item, result);
			}	else {	
				DEBUG_ITEM(item->itemid, "Executing as Javascript");
...
ZBX_THREAD_ENTRY(poller_thread, args)
{
	...
	int			nextcheck, sleeptime = -1, processed = 0, old_processed = 0,  old_total_sec = 0;
...
#define	STAT_INTERVAL	5	/* if a process is busy and does not sleep then update status not faster than */
	...
	while (ZBX_IS_RUNNING())
	{
		...
		if (0 != sleeptime)
		{
			...
			glb_state_procstat_update(server_num, process_num, get_process_type_string(process_type));
		}
