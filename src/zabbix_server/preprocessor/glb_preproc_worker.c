...
static void send_metric_to_lld(const metric_t *metric) {
    
    if ( 0 != (program_type & ZBX_PROGRAM_TYPE_PROXY)) {
        DEBUG_ITEM(metric->itemid, "Sending lld item to server sync");
...
static void preprocess_metric_execute_steps(const metric_t *metric, zbx_pp_cache_t *cache, int dep_level) {
	...
	DEBUG_ITEM(metric->itemid,"Metric entered to preprocessing with flags %"PRIu64", type %d, type %s value %s", metric->flags,
             metric->value.type, zbx_variant_type_desc(&metric->value), zbx_variant_value_desc(&metric->value));
	
    preproc_conf = get_prperoc_item(metric->itemid);
    
    //lld items will have preproc item to keep flags which we need on dependant items to avoid config cache hummering
    if ( NULL == (preproc_conf)) {
        DEBUG_ITEM(metric->itemid, "Preprocessing conf not found for item");
    ...
    metric_t new_metric = *metric;
    new_metric.value = value_out;

    if (preproc_conf->preproc->flags & ZBX_FLAG_DISCOVERY_RULE) 
        send_metric_to_lld(&new_metric);
    else
...
static void process_dependent_metrics(metric_t * metric, zbx_pp_item_preproc_t *preproc_conf, int level) {
    ...
    zbx_pp_item_t *dep_preproc_conf;
	int i;
    ...
    if (VARIANT_VALUE_NONE == metric->value.type)
        return;

    DEBUG_ITEM(metric->itemid, "Processing dependat metrics, metric value type is %d", metric->value.type);
    
    cache = pp_cache_create(preproc_conf, &metric->value);
	
    for (i = 0; i < preproc_conf->dep_itemids_num; i++ ) {
        DEBUG_ITEM(metric->itemid, "Processing dependant metric %ld", preproc_conf->dep_itemids[i]);
        DEBUG_ITEM(preproc_conf->dep_itemids[i], "Processing dependant metric %ld", preproc_conf->dep_itemids[i]);
        dep_metric.itemid = preproc_conf->dep_itemids[i];
        
        //dep_metric.flags = preproc_conf->flags;
        //if (NULL != ( dep_preproc_conf = get_prperoc_item(metric->itemid)))
        //    dep_metric.flags = dep_preproc_conf->preproc->flags;
...
static void metrics_proc_cb(const metric_t *metric) {
    
//	if (metric->flags & ZBX_FLAG_DISCOVERY_RULE) {
//		LOG_INF("In preproc: item %llu is the discovery rule", metric->itemid);
//	}
...
void proctitle_update(poller_item_t *poller_item, void *data) {
    ...
    glb_state_procstat_update(conf->server_num, conf->process_num, get_process_type_string(conf->process_type));
    conf->total_proc = 0;
}
