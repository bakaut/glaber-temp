...
int snmp_worker_discovery_need_more_data(snmp_worker_item_t *snmp_item) {
    ...
    if ( -1 != snmp_item->request_data.discovery_data->current_oid &&
          (snmp_item->request_data.discovery_data->current_oid < 
            (snmp_item->request_data.discovery_data->count)) )
        return SUCCEED;

    return FAIL;
}

static void	snmp_walk_save_result_value(poller_item_t *poller_item, snmp_worker_discovery_t *ddata, const char *index, const char *value)
{
    snmp_dobject_t	*obj;
	...
	obj->values[ddata->current_oid] = zbx_strdup(NULL,  ZBX_NULL2EMPTY_STR(value));
   // DEBUG_ITEM(poller_item_get_id(poller_item),"Added value for index '%s', value '%s', slot %d", index, value, ddata->current_oid);
}
...
int  snmp_worker_process_discovery_response(poller_item_t *poller_item, struct zbx_json_parse *jp) {
    ...
    if (ddata->current_oid == -1) {
        LOG_INF("Arrived response for item %llu in non-polled state, response is '%s' ", poller_item_get_id(poller_item), jp->start );
    ...
    DEBUG_ITEM(poller_item_get_id(poller_item), "Current Walk oid is '%s', macro is '%s'", walk_oid, macro);
    ...
    if (FAIL == zbx_json_brackets_by_name(jp, "values", &jp_values))  {
        LOG_INF("Cannot parse response for item %llu, response is '%s' ", poller_item_get_id(poller_item), jp->start );
        poller_preprocess_str(poller_item, NULL, "Cannot parse response from snmp worker");
        snmp_worker_clean_discovery_request(snmp_item);
        return FAIL;
    }
    ...
    while (p = zbx_json_next(&jp_values, p)) {
        if (FAIL == zbx_json_brackets_open(p, &jp_value) ||
            ...
            DEBUG_ITEM(poller_item_get_id(poller_item),"Failed to parse the value");
            continue;
        }
        ...
        snmp_walk_save_result_value(poller_item, ddata, idx, value);        
    }    

    if (ddata->current_oid == ddata->count - 1) {
        DEBUG_ITEM(poller_item_get_id(poller_item),"Last oid %d out of %d is polled, submitting the result", 
                                ddata->current_oid+1, ddata->count);
...
int snmp_worker_init_discovery_item(poller_item_t *poller_item, const char *key) {
    ...
    ddata->macro_names = zbx_malloc(NULL, sizeof(int) * ddata->count);
    ddata->oids = zbx_malloc(NULL, sizeof(int) * ddata->count);
    ddata->current_oid = -1;    
    ...
    for (i = 0; i < req.nparam; i += 2) {
       ...
       // DEBUG_ITEM(poller_item_get_id(poller_item), "offset is  %d, stlen: %d", offset, strlen(ddata->buffer));

        offset++;
        ddata->buffer[offset] = 0;
        ...
        // DEBUG_ITEM(poller_item_get_id(poller_item), "Parsing %d pair, macro: '%s', oid :'%s',  param[i] = '%s', param[i+1] = '%s' alloc %llu, offset %llu ", 
        //                 i, ddata->buffer + ddata->macro_names[i/2], ddata->buffer + ddata->oids[i/2], req.params[i], req.params[i+1], alloc, offset);
        
        // DEBUG_ITEM(poller_item_get_id(poller_item), "Parsing %d pair, macro: '%d', oid :'%d',  param[i] = '%p', param[i+1] = '%p' alloc %llu, offset %llu ", 
        //                 i, ddata->macro_names[i/2], ddata->oids[i/2], req.params[i], req.params[i+1], alloc, offset);
    ...
    for ( i = 0; i < ddata->count; i++) 
        DEBUG_ITEM(poller_item_get_id(poller_item), "Parsed %d pair, macro: '%s', oid :'%s'", i, ddata->buffer + ddata->macro_names[i], 
                                    ddata->buffer + ddata->oids[i] );
...
void snmp_worker_start_discovery_next_walk(poller_item_t *poller_item, const char*addr) {
    ...
    if (ddata->current_oid >= ddata->count) {
        DEBUG_ITEM( poller_item_get_id(poller_item), "Started next poll, but polling is finished ");
        snmp_worker_clean_discovery_request(snmp_item);
    ...
    poller_reset_lastpolltime(poller_item);

    zbx_snprintf(request, MAX_STRING_LEN, "{\"id\":%llu, \"ip\":\"%s\", \"oid\":\"%s\", \"type\":\"walk\", %s}", 
                poller_item_get_id(poller_item),
                ddata->address, 
                ddata->buffer + ddata->oids[ddata->current_oid],
