...
static void  snmp_worker_clean_specific_request_data(snmp_worker_item_t *snmp_item ) {
  ...
  //          snmp_worker_clean_get_request(snmp_item);
            break;
    }
}
...
static void snmp_worker_finish_poll(poller_item_t *poller_item)
{
    ...
    snmp_worker_clean_specific_request_data(snmp_item);
...
static void process_result(const char *response)
{
    ...
    DEBUG_ITEM(itemid, "Parsed itemid result code %llu ", code);
...
const char *snmp_worker_parse_oid(const char *in_oid)
{
    ...
    for (i = 0; i < oid_len; i++)
        pos += zbx_snprintf(buffer + pos, MAX_OID_LEN - pos, ".%d", (int)p_oid[i]);
