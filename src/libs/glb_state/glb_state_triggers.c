...
void glb_state_triggers_apply_diffs(zbx_vector_ptr_t *trigger_diff)
{
	...
	for (i = 0; i < trigger_diff->values_num; i++)
	{
     ...
     //       continue;
        ...
        DEBUG_TRIGGER(diff->triggerid,"Saving trigger state to state cache: value %d, lastcalc: %d, error: '%s'",
...
DUMPER_TO_JSON(trigger_to_json)
{
    ...
    DEBUG_TRIGGER(id, "Added trapper into json: id: %llu, value: %d, lastchange: %d, lastcalc: %d, error: %s",
