...
ELEMS_CREATE(item_create_cb)
{
    ...
    i_data->history_value_type = ITEM_VALUE_TYPE_NONE;
...
INTERNAL_METRIC_CALLBACK(items_cache_stat_cb) {
    size_t len = 0, offset = 0;
        zbx_snprintf_alloc(result, &len, &offset, 
         ...
         " \"db_hits\":%d, \"db_misses\":%d,  \"db_fails\":%d}", 
...
static int convert_metric_to_cache_value(glb_state_item_value_t *cache_val, ZBX_DC_HISTORY *h)
{
    item_variant_clear(&cache_val->value);

    cache_val->value = h->metric.value;

    if (VARIANT_VALUE_STR == h->metric.value.type) {
        char *tmp_str = h->metric.value.data.str;
        
        if (NULL == tmp_str)
            tmp_str = "";

        int len = strlen(h->metric.value.data.str) + 1;
        char *str = state->memf.malloc_func(NULL, len);
    ...
    cache_val->time_sec = h->metric.ts.sec;
}
...
static int fetch_from_db_by_count(u_int64_t itemid, item_elem_t *elm, int count, int head_time, int now )
{
    ...
    if (SUCCEED == (ret = glb_history_get_history(itemid, elm->history_value_type, 0, count, head_time, GLB_HISTORY_GET_NON_INTERACTIVE, &values)))
    {
        ...
        if (values.values_num > 0)
        {
            ...
            for (i = 0; i < values.values_num; i++)
            {
                if (NULL != (c_val = glb_tsbuff_add_to_tail(&elm->tsbuff, values.values[i].timestamp.sec)))
                    hist_val_to_value(c_val, &values.values[i].value, values.values[i].timestamp.sec, elm->history_value_type);
            }
        }
        else
            elm->db_fetched_time = head_time;

        zbx_history_record_vector_destroy(&values, elm->history_value_type);
        return SUCCEED;
    }
...
static int glb_state_fetch_from_db_by_time(u_int64_t itemid, item_elem_t *elm, int seconds, int head_time, int now)
{
    ...
    if (SUCCEED == (ret = glb_history_get_history(itemid, elm->history_value_type, head_time - seconds, 0,
...
static int fill_items_values_by_index(u_int64_t itemid, item_elem_t *elm, int tail_idx, int head_idx, zbx_vector_history_record_t *values)
{
    ...
    for (i = tail_idx; iter == 0 || i != glb_tsbuff_index(&elm->tsbuff, head_idx + 1); i = glb_tsbuff_index(&elm->tsbuff, i + 1))
    {
        ...
        glb_state_value_to_hist_copy(&record, item_val, elm->history_value_type);
...
static int search_head_idx_by_time_shift(glb_tsbuff_t *tsbuff, int time_shift, int now) {
    int head_idx = -1;
    
    if (0 == time_shift)
        head_idx = tsbuff->head;
    else
        head_idx = glb_tsbuff_find_time_idx(tsbuff, now - time_shift);
...
int cache_fetch_count_cb(elems_hash_elem_t *elem, mem_funcs_t *memf,  void *req_data)
{
    ...
    DEBUG_ITEM(elem->id, "Fetch: item cache meta: count_demand:%d, time_demand:%d", elm->demand.count, elm->demand.period);
    DEBUG_ITEM(elem->id, "Fetching from the cache, item %ld has %d values, oldest(tail) value ts: %d, newest(head) value ts: %d, \n Request: head ts: %d, count %d, second: %d", 
            elem->id,  glb_tsbuff_get_count(&elm->tsbuff), glb_tsbuff_get_time_tail(&elm->tsbuff), glb_tsbuff_get_time_head(&elm->tsbuff),
            req->time_shift, req->count, req->seconds  );
 
    int head_idx = -1, need_count = 0;
 
    if (req->value_type != elm->history_value_type)
    {
        
        if (glb_tsbuff_get_count(&elm->tsbuff) > 0 ) {
            DEBUG_ITEM(elem->id,"Item type mismactch: current: %d, new %d, recreating the item", elm->history_value_type, req->value_type);
            free_item(elm);
            item_create_cb(elem, memf, NULL);
            elm = (item_elem_t *)elem->data;
        }
        
        elm->history_value_type = ITEM_VALUE_TYPE_NONE;
        DC_get_item_history_value_type(elem->id, &elm->history_value_type);
    }

    int cache_start = glb_tsbuff_get_time_head(&elm->tsbuff);
   
    DEBUG_ITEM(elem->id, "Cache recent ts is %d, request time shift is %d", cache_start, req->time_shift);
 
    head_idx = search_head_idx_by_time_shift(&elm->tsbuff, req->time_shift, now);

    DEBUG_ITEM(elem->id, "head idx is %d, check if has enough data %d", 
        head_idx, SUCCEED == glb_tsbuff_check_has_enough_count_data_idx(&elm->tsbuff, req->count, head_idx));
    ...
    if (-1 == head_idx)
    {
        ...
        DEBUG_ITEM(elem->id, "Requesting item from the DB by time %d->%d", now-req->time_shift, now);

        if (FAIL == glb_state_fetch_from_db_by_time(req->itemid, elm, now - req->time_shift, now, now))
        {
            count_db_fail();
            return FAIL;
        }

        update_db_fetch(elm, now - req->time_shift);
    }
    ...
    {
      //  count_db_requests();
        DEBUG_ITEM(elem->id, "Requesting item from the DB by count starting at %d,  %d items", req->time_shift, need_count);

       if (SUCCEED != fetch_from_db_by_count(req->itemid, elm, need_count, now - req->time_shift, now))
        {
            count_db_fail();
            return FAIL;
        }
...
static int cache_fetch_time_cb(elems_hash_elem_t *elem, mem_funcs_t *memf, void *req_data)
{
    ...
    if (req->value_type != elm->history_value_type)
    {
        DEBUG_ITEM(elem->id, "Resetting value type %d -> %d ", elm->history_value_type, req->value_type);

        free_item(elm);
        item_create_cb(elem, memf, NULL);

        elm = (item_elem_t *)elem->data;
        elm->history_value_type = req->value_type;
    }
  
    head_idx = search_head_idx_by_time_shift(&elm->tsbuff, req->time_shift, now);
    tail_idx = glb_tsbuff_find_time_idx(&elm->tsbuff, req->time_shift - req->seconds);
    ...
    if (glb_tsbuff_get_count(&elm->tsbuff) > 0 &&
        (now - req->time_shift) >= glb_tsbuff_get_time_tail(&elm->tsbuff))
    {

        fetch_head_time = glb_tsbuff_get_time_tail(&elm->tsbuff) + 1;
        fetch_seconds = req->seconds - (now - req->time_shift - fetch_head_time);
    ...
    update_db_fetch(elm, req->time_shift);

    // looking for start/end againg
    head_idx = search_head_idx_by_time_shift(&elm->tsbuff, req->time_shift, now);
    tail_idx = glb_tsbuff_find_time_idx(&elm->tsbuff, now - req->time_shift - req->seconds);
    ...
    if (-1 == tail_idx)
    {
        ...
        DEBUG_ITEM(elem->id, "DB request successful, but still not enough data to fullfill the demand (has %d seconds, need %d seconds), FAILing",
                   now - glb_tsbuff_get_time_tail(&elm->tsbuff), req->seconds + (now - req->time_shift));
...
static int add_metric_lld_cb(elems_hash_elem_t *elem, mem_funcs_t *memf,  void *data) {
    glb_state_item_value_t *c_val;
    item_elem_t *elm = elem->data;
    ...
    if (NULL != (c_val = (glb_state_item_value_t *)glb_tsbuff_add_to_head(&elm->tsbuff, h->metric.ts.sec)))
    {
        convert_metric_to_cache_value(c_val, h);
        elm->history_value_type = h->hist_value_type;
       DEBUG_ITEM(elem->id, "Item added to LLD, oldest timestamp is %d", glb_tsbuff_get_time_tail(&elm->tsbuff));
    }
    else
    {
        LOG_WRN("Cannot add LLD value for item %ld with timestamp %d to the cache", h->metric.itemid, h->metric.ts.sec);
        return FAIL;
    }
}
...
int add_value_cb(elems_hash_elem_t *elem, mem_funcs_t *memf,  void *data)
{
    ...
    if (VARIANT_VALUE_ERROR == h->metric.value.type ||
        ...
        ITEM_VALUE_TYPE_NONE == h->hist_value_type) {
        DEBUG_ITEM(h->metric.itemid, "Not adding value to ValueCache: doesn't have a real value");
        return FAIL;
    }

    if (elm->meta.lastdata < h->metric.ts.sec)
        elm->meta.lastdata = h->metric.ts.sec;
    ...
    DEBUG_ITEM(elem->id, "Adding value to value cache, time %d (offset %d) hist type %d, variant type %d, value '%s'",
                        h->metric.ts.sec, (now - h->metric.ts.sec), h->hist_value_type, h->metric.value.type, zbx_variant_value_desc(&h->metric.value));

    if (elm->history_value_type == ITEM_VALUE_TYPE_NONE) {
        elm->history_value_type = h->hist_value_type;
    }

    if (elm->history_value_type != h->hist_value_type)
    {
        DEBUG_ITEM(elem->id, "Resetting value type %d -> %d ", elm->history_value_type, h->hist_value_type);
        free_item(elm);
        item_create_cb(elem, memf, NULL);
        elm = (item_elem_t *)elem->data;
        elm->history_value_type = h->hist_value_type;
    ...
    {
        ensure_tsbuff_has_space(elm);
        //DEBUG_ITEM(elem->id, "After ensuring free space there %d items, tsbuff size is %d", elm->tsbuff.count, elm->tsbuff.size );
    }

    if (h->metric.ts.sec > now + 300)
    {
        DEBUG_ITEM(elem->id, "Warn: item's imestamp %ld is too much ahead of the real time %ld, not adding to the cache",  
                        h->metric.ts.sec, now);
        return FAIL;
    }
    ...
    {
        DEBUG_ITEM(h->metric.itemid, "Cannot add value for item with timestamp %d to the cache",  h->metric.ts.sec);
        return FAIL;
    }
...
int add_json_item_value(int value_type, struct zbx_json *json, glb_state_item_value_t *c_val)
{
    ...
    case ITEM_VALUE_TYPE_STR:
    case ITEM_VALUE_TYPE_LOG:
        zbx_json_addstring(json, "value", zbx_variant_value_desc(&c_val->value), ZBX_JSON_TYPE_STRING);
        break;
...
int item_get_values_json_cb(elems_hash_elem_t *elem, mem_funcs_t *memf,  void *cb_data)
{
    ...
    if (-1 < head_idx)
    {
        do
        {
            ...
            add_json_item_value(elm->history_value_type, req->json, c_val);
...
static int parse_json_item_fields(struct zbx_json_parse *jp, item_elem_t *elm)
{
    ...
    elm->history_value_type = temp;

    glb_json_get_int_value_by_name(jp, "db_fetched_time", &elm->db_fetched_time);

    if ((elm->history_value_type >= ITEM_VALUE_TYPE_MAX || elm->history_value_type < 0) && elm->history_value_type != ITEM_VALUE_TYPE_NONE) {
        LOG_WRN("Imporeper value type is set in the file: %d: '%s'", elm->history_value_type, jp->start);
        return FAIL;
    }
...
int json_to_hist_record(struct zbx_json_parse *jp, unsigned char value_type, ZBX_DC_HISTORY *hist)
{
    ...
    if (FAIL == glb_json_get_int_value_by_name(jp, "clock", &hist->metric.ts.sec))
        return FAIL;

    hist->metric.ts.ns = 0;
    hist->hist_value_type = value_type;
    zbx_variant_clear(&hist->metric.value);

    switch (value_type)
    {
    case ITEM_VALUE_TYPE_UINT64:

        if (SUCCEED == glb_json_get_uint64_value_by_name(jp, "value", &uint64_val))
            zbx_variant_set_ui64(&hist->metric.value, uint64_val);

        break;
    case ITEM_VALUE_TYPE_FLOAT:
        if (SUCCEED == glb_json_get_dbl_value_by_name(jp, "value", &dbl_val)) 
            zbx_variant_set_dbl(&hist->metric.value, dbl_val);
        break;
    case ITEM_VALUE_TYPE_STR:
    case ITEM_VALUE_TYPE_TEXT:
    case ITEM_VALUE_TYPE_LOG:
        if (SUCCEED == zbx_json_value_by_name_dyn(jp, "value", &str_value, &alloc, &type))
            zbx_variant_set_str(&hist->metric.value, str_value);
        break;
...
DUMPER_FROM_JSON(unmarshall_item_cb)
{
    ...
    DEBUG_ITEM(elem->id, "Loaded item value type %d", elm->history_value_type);
...
int  glb_state_item_add_lld_value(u_int64_t itemid, zbx_timespec_t *ts, char *value ) {
    ...
    ZBX_DC_HISTORY h = {0};
    h.metric.itemid = itemid;
    h.metric.value.type = VARIANT_VALUE_STR;
    h.metric.value.data.str = value;
    h.hist_value_type = ITEM_VALUE_TYPE_TEXT;
    h.metric.ts = *ts;

    DEBUG_ITEM(itemid,"Adding LLD value to the state cache");

    return elems_hash_process(state->items, itemid, add_metric_lld_cb, &h, 0);
}

int	zbx_vc_get_values(zbx_uint64_t itemid, int value_type, zbx_vector_history_record_t *values, int seconds,
		int count, int time_shift) {
     
    DEBUG_ITEM(itemid, "Cache request: multiple items reqeuest: secodns:%d, count:%d, shift:%d ", seconds, count, time_shift);
	return glb_state_item_get_values(itemid, value_type, values, seconds, count, time_shift);
}

int	zbx_vc_get_value(zbx_uint64_t itemid, int value_type, int time_shift, zbx_history_record_t *value) {
	zbx_vector_history_record_t	values;

	zbx_history_record_vector_create(&values);
	DEBUG_ITEM(itemid, "Cache request: single value with time shift %d",time_shift);


	if (SUCCEED != glb_state_item_get_values(itemid, value_type, &values, 0, 1, time_shift)) {
...
int  glb_state_item_add_values(ZBX_DC_HISTORY *history, int history_num) {
    ...
    for (i = 0; i < history_num; i++)
    {
        ...
        DEBUG_ITEM(h->metric.itemid, "Adding to value cache, flags is %d type: '%s', value '%s'", 
                h->metric.flags, zbx_variant_type_desc(&h->metric.value), zbx_variant_value_desc(&h->metric.value));
        
        if (VARIANT_VALUE_ERROR == h->metric.value.type)
        {
            DEBUG_ITEM(h->metric.itemid, "Added error '%s' as state error", h->metric.value.data.str);
            elems_hash_process(state->items, h->metric.itemid, item_set_error_cb, ZBX_NULL2EMPTY_STR(h->metric.value.data.str), 0);
      
            continue;
        }

        ret = elems_hash_process(state->items, h->metric.itemid, add_value_cb, h, 0);
        DEBUG_ITEM(h->metric.itemid, "Added value with timestamp %d to the cache", h->metric.ts.sec);
    }
...
int  glb_state_item_get_values( u_int64_t itemid, int value_type, zbx_vector_history_record_t *values, int seconds, int count, int time_shift) {
	
    DEBUG_ITEM(itemid,"In %s:starting itemid: %ld, count: %d, time_shift:%d", __func__,itemid, count, time_shift );

	if (count > 0)  {
        fetch_req_t req = {.itemid = itemid, .count = count, .value_type = value_type, .values = values, .time_shift = time_shift};
    	
        return elems_hash_process(state->items, itemid, cache_fetch_count_cb, &req, ELEM_FLAG_DO_NOT_CREATE);
	}

    fetch_req_t req = {.itemid = itemid, .value_type = value_type, .values = values, .seconds = seconds, .time_shift = time_shift };
...
{   int head_idx;
    ...
    zbx_json_addint64(json, "value_type", elm->history_value_type);
    ...
    if (-1 < tail_idx)
    {
        ...
        do
        {
            ...
            add_json_item_value(elm->history_value_type, json, c_val);
...
ELEMS_CALLBACK(get_valuetype_cb) {
     ...
     return item->history_value_type;
}
...
ELEMS_CALLBACK(get_cached_items) {
    item_elem_t *item=elem->data;
    item->meta.lastlogsize = *(int*) data;
    int *count = data;

    *count = glb_tsbuff_get_count(&item->tsbuff);
    return SUCCEED;
}

int glb_state_item_get_cached_values_count(u_int64_t itemid) {
    int count = 0;
    elems_hash_process(state->items, itemid, get_cached_items, &count, ELEM_FLAG_DO_NOT_CREATE);

    return count;
}

ELEMS_CALLBACK(dump_item_to_json) {
    item_to_json_cb(elem->id, (item_elem_t *)elem->data, (struct zbx_json*)data);
    return SUCCEED;
}

void glb_state_item_cache_dump(u_int64_t itemid) {
    struct zbx_json j;
    zbx_json_init(&j, 2048);
    elems_hash_process(state->items, itemid, dump_item_to_json, &j, ELEM_FLAG_DO_NOT_CREATE );
    LOG_INF("%s", j.buffer);
    zbx_json_free(&j);
}