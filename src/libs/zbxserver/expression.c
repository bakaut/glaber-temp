...
static int	DBget_item_value(zbx_uint64_t itemid, char **replace_to, int request)
{
	...
	switch (request)
	{
		...
		case ZBX_REQUEST_ITEM_ID:
				...
				zbx_snprintf(buffer, MAX_ID_LEN, "%llu", itemid);
		...
		const char *tz)
{
	...
	if (SUCCEED != zbx_vc_get_value(itemid, item.value_type, 0, &value))
		goto out;
...
static int	DBitem_get_value(zbx_uint64_t itemid, char **lastvalue, int raw, zbx_timespec_t *ts)
{
	...
	DEBUG_ITEM(itemid, "Called %s", __func__);
	zabbix_log(LOG_LEVEL_DEBUG, "In %s()", __func__);
	...
	if (SUCCEED == DC_get_item_valuetype_valuemapid(itemid, &valuemapid, &value_type, &units))
	{
		...
		DEBUG_ITEM(itemid, "Got item valuem valuemapid %llu, value_type $d", valuemapid, value_type);
		zbx_history_record_t	vc_value;

		if (SUCCEED == zbx_vc_get_value(itemid, value_type, 0, &vc_value))
		{
			char	tmp[MAX_BUFFER_LEN];

			DEBUG_ITEM(itemid, "Got item from value cache type");
			...
			*lastvalue = zbx_strdup(*lastvalue, tmp);
			...
			DEBUG_ITEM(itemid, "Cannot get item value from value cache");
	} else 
		DEBUG_ITEM(itemid, "Cannot get item valuemapid and valuetype");
...
static int	DBitem_value(const zbx_db_trigger *trigger, char **value, int N_functionid, int clock, int ns, int raw)
{
	...
	if (SUCCEED == (ret = zbx_db_trigger_get_itemid(trigger, N_functionid, &itemid))) {
		DEBUG_TRIGGER(trigger->triggerid, "Found item %llu, N is %d for macro calculation for expression '%s'", itemid, N_functionid, trigger->expression);
		ret = DBitem_get_value(itemid, value, raw, &ts);
	} else 
		DEBUG_TRIGGER(trigger->triggerid, "Cannot find item%d for macro calculation for expression '%s'", N_functionid, trigger->expression);
		...
		zbx_history_sync_item_t **items, int **items_err, int *items_num)
{
	...
	//	}
		...
		if (ITEM_STATE_NOTSUPPORTED == glb_state_item_get_oper_state(item->itemid) &&
...
static int	substitute_expression_functions_results(zbx_hashset_t *ifuncs, zbx_eval_context_t *ctx, char **error, u_int64_t triggerid)
{
	zbx_uint64_t		functionid;
	zbx_func_t		*func;
	zbx_ifunc_t		*ifunc;
	...
	for (i = 0; i < ctx->stack.values_num; i++)
	{
		...
		if (ZBX_EVAL_TOKEN_FUNCTIONID != token->type)
			continue;
		DEBUG_TRIGGER(triggerid, "Calculating function token type is '%s', token value is '%s'", 
					zbx_variant_type_desc(&token->value), zbx_variant_value_desc(&token->value) );
...
static void	zbx_substitute_functions_results(zbx_hashset_t *ifuncs, zbx_vector_ptr_t *triggers)
{
	...
	for (i = 0; i < triggers->values_num; i++)
	{
		...
		DEBUG_TRIGGER(tr->triggerid, "calculating functions in the trigger");

		if( SUCCEED != substitute_expression_functions_results(ifuncs, tr->eval_ctx, &tr->new_error, tr->triggerid))
		{
		...
		if (TRIGGER_RECOVERY_MODE_RECOVERY_EXPRESSION == tr->recovery_mode)
		{
			if (SUCCEED != substitute_expression_functions_results(ifuncs, tr->eval_ctx_r, &tr->new_error, tr->triggerid))
			{
				tr->new_value = TRIGGER_VALUE_UNKNOWN;
				continue;
			}
		...
		char *error, size_t maxerrlen)
{
	...
	switch (macro_type)
	{
		...
		//we treat item key as fully pasrable and replaceble
		//so it's just like an snmp item
