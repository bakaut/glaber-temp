...
{
	int				arg1 = 1, ret = FAIL, time_shift = 0 ;
	...
	if (SUCCEED != zbx_vc_get_values(item->itemid, item->value_type, &values, 0, arg1, time_shift))
	{
		...
		const zbx_timespec_t *ts, int limit, int unique, char **error)
{
	...
	if (FAIL == zbx_vc_get_values(item->itemid, item->value_type, &values, seconds, nvalues, time_shift))
	{
		*error = zbx_strdup(*error, "cannot get values from value cache");
		goto out;
	}
...
static int	evaluate_NODATA(zbx_variant_t *value, const DC_EVALUATE_ITEM *item, const char *parameters, char **error)
{
	...
	if (SUCCEED != (ret = glb_state_item_get_values(item->itemid, item->value_type, &values, 0, 1, 0))) {
...
static int	evaluate_CHANGE(zbx_variant_t *value, const DC_EVALUATE_ITEM *item, const zbx_timespec_t *ts, char **error)
{
	...
	if (SUCCEED != zbx_vc_get_values(item->itemid, item->value_type, &values, 0, 2, 0) ||
			2 > values.values_num)
	{
		*error = zbx_strdup(*error, "cannot get values from value cache");
		goto out;
	...
	{
		case ITEM_VALUE_TYPE_FLOAT:
			result = values.values[1].value.dbl - values.values[0].value.dbl;
			break;
		case ITEM_VALUE_TYPE_UINT64:
			result = (double)values.values[1].value.ui64 - (double)values.values[0].value.ui64;
			break;
		case ITEM_VALUE_TYPE_LOG:
		...
		const zbx_timespec_t *ts, char **error)
{
	...
	if (SUCCEED != zbx_vc_get_value(item->itemid, item->value_type, 0, &vc_value))
	{
		...
		const zbx_timespec_t *ts, char **error)
{
	...
	if (SUCCEED == zbx_vc_get_values(item->itemid, item->value_type, &values, seconds, 0, time_shift))
	{
		if (0 < values.values_num)
		{
		...
		const zbx_timespec_t *ts, char **error)
{
	...
	if (SUCCEED != zbx_vc_get_values(item->itemid, item->value_type, &values, seconds, nvalues, time_shift))
	{
		*error = zbx_strdup(*error, "cannot get values from value cache");
		goto out;
	}
