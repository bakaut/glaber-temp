...
// 		if (ITEM_STATE_NOTSUPPORTED == h->oper_state)
// 			continue;

// 		if (ITEM_VALUE_TYPE_LOG != h->hist_value_type)
// 			continue;
...
// 			zbx_db_insert_add_values(&db_insert, h->metric.itemid, h->metric.ts.sec, h->metric.ts.ns, log->timestamp,
...
// 			zbx_db_insert_add_values(&db_insert, h->metric.itemid, h->metric.ts.sec, h->metric.ts.ns, unset_if_novalue, "",
// 									 unset_if_novalue, "", unset_if_novalue, 0, 0, flags, now);
								...
								zbx_vector_ptr_t *inventory_values, zbx_vector_uint64_pair_t *proxy_subscribtions)
{
	...
	for (i = 0; i < history_num; i++)
	{
		...
		if (ITEM_ADMIN_STATUS_ENABLED != item->status || HOST_STATUS_MONITORED != item->host.status)
		{
		...
		{
			zbx_uint64_pair_t p = {item->host.proxy_hostid, h->metric.ts.sec};
...
static void metrics_proc_cb(const metric_t * metric, int i, void *cb_data) {
	...
	//if (VARIANT_VALUE_ERROR == h->metric.value.type) 
		///h->oper_state = 
	//history->oper_state = ITEM_STATE_NOTSUPPORTED;
