            ...
            if (SUCCEED == zbx_json_value_by_name(&jp_row, "time_sec", clck,MAX_ID_LEN, &type) )
				...
				switch (value_type)
				{
					case ITEM_VALUE_TYPE_UINT64:
						DEBUG_ITEM(itemid, "Parsed as clock %d UINT64 %s",hr.timestamp.sec, value);
					...
					case ITEM_VALUE_TYPE_FLOAT: 
						...
						DEBUG_ITEM(itemid, "Parsed as clock %d DBL %s",hr.timestamp.sec, value);
			    		hr.value = history_str2value(value, value_type);
                        zbx_vector_history_record_append_ptr(values, &hr);
						break;
...
static int	worker_add_history(void *data, ZBX_DC_HISTORY *hist, int history_num)
{
	...
	for (i = 0; i < history_num; i++)
	{
		...
		if (VARIANT_VALUE_ERROR == h->metric.value.type || 0 != (h->metric.flags & (ZBX_DC_FLAG_UNDEF | ZBX_DC_FLAG_NOHISTORY))) {
			DEBUG_ITEM(h->metric.itemid, "Not saving item's history to worker, flags are %u", h->metric.flags);
			continue;
		}
		...
		zbx_json_adduint64(&json,"itemid",h->metric.itemid);
		zbx_json_adduint64(&json,"time_sec",h->metric.ts.sec);
		zbx_json_adduint64(&json,"time_ns",h->metric.ts.ns);
		zbx_json_addint64(&json,"value_type", h->hist_value_type);

		switch (h->hist_value_type) {
			case ITEM_VALUE_TYPE_UINT64:
	    		zbx_json_addstring(&json, "value_int", zbx_variant_value_desc(&h->metric.value), ZBX_JSON_TYPE_STRING);
				DEBUG_ITEM(h->metric.itemid, "Sending item to the history storage UINT64 %s", zbx_variant_value_desc(&h->metric.value));
				break;
			case ITEM_VALUE_TYPE_FLOAT: 
				DEBUG_ITEM(h->metric.itemid, "Sending item to the history storage DOUBLE %s", zbx_variant_value_desc(&h->metric.value));
		   		zbx_json_addstring (&json, "value_dbl", zbx_variant_value_desc(&h->metric.value), ZBX_JSON_TYPE_STRING);
				break;
			case ITEM_VALUE_TYPE_LOG:
//				if (h->value.log) {
...
//				break;
			case ITEM_VALUE_TYPE_STR:
			case ITEM_VALUE_TYPE_TEXT:
				zbx_json_addstring(&json, "value_str", zbx_variant_value_desc(&h->metric.value), ZBX_JSON_TYPE_STRING);
				break;
			default:
