...
void run_many_senders_test(){
 ...
 void check_buffer_metric_cb(void *buff, void *item, void *ctx_data, int i) {
   ...
   // LOG_INF("Metrics's addr is %llu", item);

    LOG_INF("Metric host id is %llu, itemid %llu", metric->hostid, metric->itemid);
...
void integrity_check_cb(void *buff, void *item, void *ctx_data, int i) {
    ...
    LOG_INF("Got %d, got item %llu, length %d", i, m->itemid, strlen(m->value.data.str));
}
