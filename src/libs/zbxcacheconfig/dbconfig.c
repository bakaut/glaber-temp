	...
	switch (zbx_dc_item->type) {
		...
		case ITEM_TYPE_SNMP: {
			...
			*snmp_version = snmp_iface->version;
			...
			//walk[ are only implemented in worker or classic poller
			if ( CONFIG_FORKS[GLB_PROCESS_TYPE_SNMP_WORKER] == 0 && 0 == strncmp(snmpitem->snmp_oid,"walk[", strlen("walk[")))
				return FAIL;
							...
							char **error)
{
	...
	if (0 != (flags & ZBX_ITEM_NEW) &&
		...
		ITEM_TYPE_AGENT_ACTIVE != item->type &&
...
void DCupdate_item_queue(ZBX_DC_ITEM *item, unsigned char old_poller_type)
{
	...
	if (ZBX_LOC_QUEUE != item->location)
	{
		...
		DEBUG_ITEM(item->itemid, "Item's next check is in %ld", item->queue_next_check - time(NULL));
...
{
	if ((NULL != interface) && ((ITEM_TYPE_AGENT == type && INTERFACE_TYPE_AGENT == interface->type) ||
						 ...
						 zbx_vector_uint64_t *deleted_itemids)
{
	...
	while (SUCCEED == (ret = zbx_dbsync_next(sync, &rowid, &row, &tag)))
	{
		...
		DEBUG_ITEM(itemid, "Item is being synced");
...
static void dc_trigger_update_cache(void)
{
	...
	while (NULL != (function = (ZBX_DC_FUNCTION *)zbx_hashset_iter_next(&iter)))
	{
		...
		if (ITEM_ADMIN_STATUS_DISABLED == item->status ||
...
static void poller_item_notify_changed_hosts(zbx_vector_uint64_t *changed_hosts) {
	poller_item_notify_init();
	int i, j;
	RDLOCK_CACHE;
	for (i=0; i<changed_hosts->values_num; i++) {
		ZBX_DC_HOST *host;
		ZBX_DC_ITEM *item;
		u_int64_t hostid = changed_hosts->values[i];

		if (NULL == (host = zbx_hashset_search(&config->hosts, &hostid)))
			continue;
		
		for (j = 0; j < host->items.values_num; j++) {
			item = host->items.values[j];

			int snmp_version;

			if (SUCCEED == glb_might_be_async_polled(item, host, &snmp_version) ) {
				poller_item_add_notify(item->type, item->key, item->itemid, host->hostid, snmp_version);
				DEBUG_HOST(hostid, "Will notify host %"PRIu64", item %"PRIu64" about change due to host macro has changed", hostid, item->itemid);
				DEBUG_ITEM(item->itemid, "Will notify host %"PRIu64" item %"PRIu64" about change due to host macro has changed", hostid, item->itemid);
			}
		}
						  ...
						  const zbx_config_vault_t *config_vault)
{
	...
	zbx_vector_uint64_t active_avail_diff, changed_hosts;
...
clean:
	...
	if (ZBX_DBSYNC_INIT != mode) {
 		//LOG_INF("Sync finished %d hosts changed, doing notify", changed_hosts.values_num);
		poller_item_notify_changed_hosts(&changed_hosts);
	}
...
int DCconfig_get_itemid_by_item_key_hostid(u_int64_t hostid, char *key, u_int64_t *itemid, u_int64_t *flags)
{
 ...
 * and item names
 * ***************************************************/
int DCconfig_get_itemid_by_key(zbx_host_key_t *key, zbx_uint64_pair_t *host_item_ids, u_int64_t *flags)
{
	const ZBX_DC_ITEM *dc_item;
	const ZBX_DC_HOST *dc_host;
	int errcode = SUCCEED;
...
int DCconfig_get_active_items_count_by_hostid(zbx_uint64_t hostid)
{
	...
	if (NULL != (dc_host = (ZBX_DC_HOST *)zbx_hashset_search(&config->hosts, &hostid)))
	{
		...
		{
			if (ITEM_TYPE_AGENT_ACTIVE == dc_host->items.values[i]->type)
				num++;
		}
	}
...
void DCconfig_get_active_items_by_hostid(DC_ITEM *items, zbx_uint64_t hostid, int *errcodes, size_t num)
{
		...
		0 != dc_host->items.values_num)
	{
		for (i = 0; i < (size_t)dc_host->items.values_num && j < num; i++)
		{
			...
			if (ITEM_TYPE_AGENT_ACTIVE != dc_item->type)
				continue;
...
static void	dc_preproc_sync_item(zbx_hashset_t *items, ZBX_DC_ITEM *dc_item, zbx_uint64_t revision)
{
	...
	if (DC_get_debug_item() == pp_item->itemid) {
		...
		DEBUG_ITEM(pp_item->itemid,"Synced preproc config to preprocessor: steps %d, depends %d, flags %d", 
					pp_item->preproc->steps_num, pp_item->preproc->dep_itemids_num, pp_item->preproc->flags);
	...
	if (NULL != dc_item->master_item)
	{
		...
		for (i = 0; i < dc_item->master_item->dep_itemids.values_num; i++)
		{
			...
			if (NULL == (dep_item = (ZBX_DC_ITEM *)zbx_hashset_search(&config->items,
					...
					ITEM_ADMIN_STATUS_ENABLED != dep_item->status)
			{
				continue;
			}
...
void DCconfig_get_preprocessable_items(zbx_hashset_t *items, zbx_uint64_t *revision, int manager_num)
{
	...
	while (NULL != (dc_host = (ZBX_DC_HOST *)zbx_hashset_iter_next(&iter)))
	{
		...
		for (i = 0; i < dc_host->items.values_num; i++)
		{
			...
			//item alive check
			if (ITEM_ADMIN_STATUS_ENABLED != dc_item->status || ITEM_TYPE_DEPENDENT == dc_item->type)
				continue;
								   ...
								   const zbx_vector_ptr_t *timers)
{
	...
	for (i = 0; i < timers->values_num; i++)
	{
		...
		if (NULL != (dc_trigger = (ZBX_DC_TRIGGER *)zbx_hashset_search(&config->triggers, &timer->triggerid)))
		{
			...
			if (NULL != zbx_hashset_search(trigger_info, &dc_trigger->triggerid)) 
					continue;
...
void zbx_dc_get_trigger_timers(zbx_vector_ptr_t *timers, int now, int soft_limit, int hard_limit)
{
	...
	while (SUCCEED != zbx_binary_heap_empty(&config->trigger_queue) && timers->values_num < hard_limit)
	{
		...
		DEBUG_TRIGGER(timer->triggerid, "Got trigger from timer queue, will exec in %d seconds", timer->exec_ts.sec - now);
...
int DCconfig_get_poller_items(unsigned char poller_type, int config_timeout, DC_ITEM **items)
{
	...
	//		poller_type, iface_avail, disabled_until);
		...
		DEBUG_ITEM(itemid, "Poller type is %d Item interface id %"PRIu64" avail is %d, disabled till is %d", 
	...
	{ //&& ( glb_ms_time() - 1000 ) < start_time ) {
			...
			NULL != (zbx_dc_host = (ZBX_DC_HOST *)zbx_hashset_search(&config->hosts, &zbx_dc_item->hostid)))
		{
			...
			if (HOST_STATUS_MONITORED != zbx_dc_host->status ||
				...
				ITEM_ADMIN_STATUS_DISABLED == zbx_dc_item->status)
			{
...
/******************************************************************************
 * TODO: there is no any reason to fully fill item and the host 
 * we only need itemid and key!
 ******************************************************************************/
size_t DCconfig_get_regexp_items_keys_by_hostid(zbx_uint64_t hostid, DC_ITEM **items)
{
	...
	if (NULL == (dc_host = (const ZBX_DC_HOST *)zbx_hashset_search(&config->hosts, &hostid)))
		goto unlock;
	...
	*items = (DC_ITEM *)zbx_malloc(*items, items_alloc * sizeof(DC_ITEM));
	...
	for (i = 0; i < dc_host->items.values_num; i++)
	{
		...
		if ( ITEM_TYPE_TRAP_REGEXP != dc_item->type)
			continue;

		if (ITEM_ADMIN_STATUS_ENABLED != dc_item->status)
			continue;
...
static void get_host_statistics(ZBX_DC_HOST *dc_host, ZBX_DC_PROXY *dc_proxy, int reset)
{
	...
	for (i = 0; i < dc_host->items.values_num; i++)
	{
		...
		{
		case ITEM_ADMIN_STATUS_ENABLED:
			if (HOST_STATUS_MONITORED == dc_host->status)
			{
				if (SUCCEED == reset)
				{
			...
			ZBX_FALLTHROUGH;
		case ITEM_ADMIN_STATUS_DISABLED:
...
void zbx_dc_reschedule_items(const zbx_vector_uint64_t *itemids, int nextcheck, zbx_uint64_t *proxy_hostids)
{
	...
	for (i = 0; i < itemids->values_num; i++)
	{
		...
		if (ZBX_JAN_2038 == dc_item->queue_next_check)
		{
...
int  DC_get_item_history_value_type(u_int64_t itemid, unsigned char *value_type) {
	ZBX_DC_ITEM *item;
	ZBX_DC_NUMITEM *numitem;
	int ret = FAIL;
