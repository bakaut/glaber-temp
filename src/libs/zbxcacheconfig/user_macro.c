...
static void	um_cache_sync_macros(zbx_um_cache_t *cache, zbx_dbsync_t *sync, int offset,
		const zbx_config_vault_t *config_vault, zbx_vector_uint64_t *changed_hosts)
{
	unsigned char		tag;
	int			ret, i;
		...
		if (NULL != (pmacro = (zbx_um_macro_t **)zbx_hashset_search(user_macros, &pmacroid)))
		{
			...
			if ((*pmacro)->hostid != hostid)
			{
				if (SUCCEED == um_macro_has_host(*pmacro) && NULL != host)
				{
					...
					um_host_remove_macro(host, *pmacro);
					zbx_vector_um_host_append(&hosts, host);
					zbx_vector_uint64_append(changed_hosts, (*pmacro)->hostid);
				}
	...
	zbx_vector_uint64_sort(changed_hosts, ZBX_DEFAULT_UINT64_COMPARE_FUNC);
		...
		zbx_vector_uint64_t *changed_hosts)
{
	...
	um_cache_sync_macros(cache, gmacros, 1, config_vault, changed_hosts);
	um_cache_sync_macros(cache, hmacros, 2, config_vault, changed_hosts);
